...
public class Aerolinea
{
    ...
    {
        // TODO implementar
    	List<Tiquete> finiti = new ArrayList<>();
    	for (int i = 0; i<vuelos.size(); i++) {
    		Vuelo vuelo = vuelos.get(i);
    		Iterator<Tiquete> iterar = vuelo.getTiquetes().iterator();
    		while (iterar.hasNext()) {
    			Tiquete tiquete = iterar.next();
    			finiti.add(tiquete);
    		}
    	}
        return finiti;
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
    	...
    	for (int x=0; x<vuelos.size();x++) {
    		Vuelo vuelo = vuelos.get(x);
    		if (vuelo.getAvion().equals(avionFini) && vuelo.getFecha().equals(fecha)) {
    			throw new Exception("Avion Ocuapdo.");
    		}		
    	}
    	
    	Vuelo nuevoVuelo = new Vuelo(ruta, fecha, avionFini);
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
    	...
    	if (vuelo == null) {
    		throw new Exception("Vuelo Inexistente");    		
    	}
    	
    	CalculadoraTarifas calculadora = new CalculadoraTarifas();
    	int fini = vuelo.venderTiquetes(cliente, calculadora, cantidad);
    ...
    {
    	Cliente cliente = getCliente(identificadorCliente);
    	int fini = cliente.calcularValorTotalTiquetes();
    	return fini + "";
